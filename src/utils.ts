...
interface CallbackResults {
  [stringyCoordinate: string]: string;
}
 ...
 */
const callbacks: {
  [callbackKey: string]: (props: CallbackProps) => string | CallbackResults;
} = {
    ...
    const data = coordinates
      .map(({ coordinate }, i) =>
        defaultArray.map((name) => {
          ...
          return [calculatedDefaultValue, coordinate];
        })
      )
      .reduce((prevValue, [[currentValue, coordinate]]) => {
        const key = JSON.stringify(coordinate);
        return { ...prevValue, [key]: currentValue as string };
      }, {} as CallbackResults);
...
async function loadPdf(): Promise<PDFDocument> {
  const arrayBuffer = await fetch(
    "/mrwetjet-docusign-alt/pdf/FINAL-WAIVER-12.30.23.pdf"
